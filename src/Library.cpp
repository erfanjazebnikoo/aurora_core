/* 
 * @File:     Library.cpp
 * @Author:   Sajjad Rahnama
 *            Erfan Jazeb Nikoo
 * 
 * @Project:  Aurora
 * @Version:  1.0 - Iran Open 2017
 * 
 * @Created on Dec 12, 2016
 */

#include "Library.h"

using namespace au;

const QString about::TITLE = "Aurora";
const QString about::VERSION = "1.0";
const QString about::TEAM = "Guilano - Professor Hesabi Robotics Research Laboratory";
const QString about::LICENCE = "No Licence";
const QString about::DESCRIPTION = "Autonomous Unmanned Aerial Robot, RoboCup Iran Open 2017, Tehran";
const QString about::PLATFORM = "Ubuntu, C++11, Qt4, ROS";
const QString about::LOGO =
  "                                                                                \n"
  "  /shhyoyy    yyyy  yys  oyyyyo .yyyys      `yyyyyy:   oyyyo  syyo   `/syhys:   \n"
  " -NMM    NN` `hMMM  MMs  /MMMd- .hMMMo      `dMmMMN-   +MMMM   MM+  -NMMh dMMm. \n"
  " yMMM  +sss-  +MMM  MM-   MMMy   +MMM.      -MM  MMs   `MM MMM MM   hMMM   MMMs \n"
  " :MMM    MM-  /MMM  MM.  -MMMh.  sMMM/ dMo -mMs:+MMM+  :MM  dMMMM   /MMMs hMMN- \n"
  "  .ohdmmds.    +hmmmy:   yddddy -dddddddd+ yddy  dddd. hddh  smmm    .ohmmmho`  \n"
  "                                                                                ";

